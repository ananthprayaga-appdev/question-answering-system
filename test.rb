...
content_request = "#{patient.fetch(:description)} The patient name is #{patient.fetch(:first_name)} #{patient.fetch(:last_name)}. Visit reason: #{patient.fetch(:sample_name)}. The notes during the visit are: #{patient.fetch(:transcription)}"
...
response = openai_client.chat(
  parameters: {
    ...
    messages: [
      { role: "system", content: content_request + "You are a helpful assistant and answer questions to the point." },
      { role: "user", content: "What are the medications she is taking?" },
    ],
    temperature: 0.7,
  },
)
